...
const ResultTemplate = ({ wpm, accuracy, rightWords, wrongWords, totalCharacter }) => {
    ...
    return (
        //boxShadow={`0px 0px 7px 0px ${cardResultShadow}`}
        <Flex w={['70%', '70%', '20%']} h='fit-content' display={wpm ? 'flex' : 'none'} bg={cardResultBg} _hover={{ boxShadow: `0px 0px 4px 1px ${cardResultShadow}` }} transitionDuration='.3s' color={textColor} justify='center' borderRadius='10px' py='10px' direction='column' role='group'>
            <Flex w='100%' justify='center' mb='10px'>
                <Flex fontWeight='bold' letterSpacing='1px' fontSize='2xl'>{wpm > 0 ? wpm.toFixed(0) : 0} WPM</Flex>
            </Flex>
            <Flex w='100%' direction='column' alignItems='center' justify='center'>
                <Flex w='80%' justify='space-between' my='3px'>
                    <Flex w='fit-content' minW='35%' justify='center' fontWeight='bold'>{accuracy > 0 ? accuracy.toFixed(2) : 0} %</Flex>
                    <Flex>Accuracy</Flex>
                </Flex>
                <Flex w='80%' justify='space-between' my='3px'>
